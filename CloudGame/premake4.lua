...
function cloud.project.useSDL()
	--
	cloud.addIncludeDir(  _OPTIONS["SDLIncludesPath"] )
	cloud.addLibDir(  _OPTIONS["SDLLibsPath"] )
	if os.get() == "windows" then
		defines { "WIN" }
		links {
		"SDL",
		...
		cloud.win.addLibFromProgrammFiles2("SDL-1.2.14", "lib")
		if  _OPTIONS["CopySharedLibraries"] then
			cloud.win.copyDLL2("SDL-1.2.14", "SDL.dll")
		end
	end
...
cloud.project.CloudGame = {}

function cloud.project.CloudGame.copyAassets()
	cwd = os.getcwd()
...
solution "CloudGame"
	...
	project "CloudGame"
		...
		cloud.project.useAL()
		cloud.project.useCV()
		cloud.project.useSDL()
		cloud.project.useASL()
		
		cloud.project.CloudGame.copyAassets()
