...
echo -------------------------------------------------------------------------------
echo --This build API:--------------------------------------------------------------
echo ./cloud_server_net_setup.sh FORCE_SVN CLOUD_INSTALL_SUBDIR BOOST_INSTALL_SUBDIR BOOST_ROOT_DIR BOOST_SETUP_FILE_NAME PREMAKE_SETUP_FILE_NAME
echo -------------------------------------------------------------------------------
...
if [ ! -e $CLOUD_LOCAL_BUILDING_FILE_NAME ]; then
	echo ../premake-4.3/bin/release/premake4 --os=$OS_NAME --BoostLibsPath=../$BOOST_ROOT_DIR/$BOOST_INSTALL_SUBDIR/lib --BoostIncludesPath=../$BOOST_ROOT_DIR/$BOOST_INSTALL_SUBDIR/include  --platform=x32 gmake > $CLOUD_LOCAL_BUILDING_FILE_NAME
	chmod u+x ./$CLOUD_LOCAL_BUILDING_FILE_NAME
fi
