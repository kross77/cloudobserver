    ...
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                ...
                ICloudObserverAuthorizationService authorizationServiceClient = new ChannelFactory<ICloudObserverAuthorizationService>(new BasicHttpBinding(), serviceAddress).CreateChannel();
                Console.WriteLine("Connection succeed. Preparing login procedure...");
                Console.Write("Email: ");
                string email = Console.ReadLine();
                Console.Write("Password: ");
                string password = Console.ReadLine();
                Console.WriteLine(authorizationServiceClient.UserLogin(email, password) ? "Login succeed." : "Login failed. Invalid email or password.");
