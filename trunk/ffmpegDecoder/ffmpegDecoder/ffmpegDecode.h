...
{ 
 ...
 public: AVInputFormat  *pInFormat;
  // constructor.
  ...
  private: int DecodeAudio(int nStreamIndex, const uint8_t* pInBuffer, size_t nInbufferSize, 
                                    uint8_t* pOutBuffer, size_t nOutBufferSize);

  // Decode video buffer.
  private: bool DecodeVideo(const uint8_t* pInBuffer, size_t nInbufferSize, AVFrame * pOutFrame);
