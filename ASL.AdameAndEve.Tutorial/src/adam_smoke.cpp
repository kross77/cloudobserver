...
{
    std::cout << "\nAdam smoke test app v" << ADOBE_VERSION_MAJOR << "." << ADOBE_VERSION_MINOR << "." << ADOBE_VERSION_SUBMINOR << std::endl << std::endl;
    std::cout << "usage: " << app_name << " [ root_path_name ]" << std::endl << std::endl;
    std::cout << "\troot_path_name specifies the root name of an Adam sheet/Known good file pair. The Adam"<< std::endl;
    std::cout << "\tsheet contains the model we would like to test, and the known good file contains"<< std::endl;
    std::cout << "\tcontributing and resulting information. When the contributing information is passed"<< std::endl;
    std::cout << "\tthrough the model, the known good resultant information should match the result. If the results" << std::endl;
...
adobe::dictionary_t read_dictionary(const bfs::path& filepath)
{
    ...
    {
        std::cout << "Could not open \"" << filepath.native_file_string() << "\"!" << std::endl;
...
void read_sheet(const bfs::path& filepath, adobe::sheet_t& sheet)
{
    ...
    {
        std::cerr << "adobe:: " << ": " << format_stream_error(input_file, error) << std::endl;
...
bool test_sheet(const bfs::path& root)
{
    ...
    for (adobe::dictionary_t::const_iterator first(contributing.begin()), last(contributing.end()); first != last; ++first)
    {
        ...
        std::cout << "Setting Cell '" << cell.c_str() << "' to " << adobe::begin_asl_cel << value << adobe::end_asl_cel<< std::endl;
    ...
    {
        std::cout << "*** PASS *** Expected value calculated!" << std::endl;
    }
    else
    {
        std::cout << "*** FAIL *** Values are different..." << std::endl;
		std::cout << "Expected Value:" << std::endl;
        std::cout << '\t' << adobe::begin_asl_cel << input_value << adobe::end_asl_cel << std::endl;
        std::cout << "Calculated Value:"<< std::endl;
        std::cout << '\t' << adobe::begin_asl_cel << sheet_value << adobe::end_asl_cel<< std::endl;
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        bfs::path sheet(bfs::path("assets/adam_smoke_rtd", bfs::native));
        ...
        {
            std::cout << "No sheet specified-- using test sheet..." << std::endl;
    ...
    {
        std::cerr << "std::exception: " << typeid(error).name() << ": " << error.what() << std::endl;
    }
    catch (...)
    {
