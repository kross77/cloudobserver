...
namespace CloudObserver.Clients.Auxiliaries.Formats.Audio.Mp3
{
    public partial struct Mp3BitRate : IEquatable<Mp3BitRate>, IComparable<Mp3BitRate>, IComparable
    {
            ...
            get
            {
                if (cachedAllValues == null)
                    lock (typeof(Mp3BitRate))
                        if (cachedAllValues == null) cachedAllValues = GetAllValuesInstance();
        ...
        {
            return new ReadOnlyCollection<Mp3BitRate>(new Mp3BitRate[]
				{
        ...
        public IEnumerable<PcmAudioFormat> CompatibleSourceFormats
        {
            get
            {
                Dictionary<PcmAudioFormat, PcmAudioFormat> result = new Dictionary<PcmAudioFormat, PcmAudioFormat>();
                foreach (MpegVersion mpegVersion in CompatibleMpegVersions)
                    foreach (PcmAudioFormat sourceFormat in mpegVersion.CompatibleSourceFormats)
        ...
        {
            get { return mpegVersions; }
        }

        public string Description
        {
            get { return string.Format("{0} kbit/sec", value); }
        }
