...
int main(void)
{
  ...
  if (!lib.is_open())
    return 2;
  //if (!lib.get<void, map<string,  reflection> &>("extension_export_car")) {
  //  return 3;
  //}
  // Call an exported function to populate
  // reflection_map
  //lib.get<void, map<string, reflection> &>("extension_export_car")(reflection_map);
  //void (*temp)(map<string, reflection>&) = lib.get<void, map<string, reflection> &>("extension_export_car");
  //(*temp)(reflection_map);
  ...
  if (reflection_map.size() != size_t(2) || reflection_map.find("suv") == reflection_map.end() || reflection_map.find("compact") == reflection_map.end())
  {
    cout << "Could not load reflections!";
    return 1;
  }

  // Pull out two reflections that were named "suv" and
  // "compact" respectively.
  reflection & first_reflection =  reflection_map["suv"];
  reflection & second_reflection =  reflection_map["compact"];

  // Use the get_constructor function to find a constructor
  // that takes one argument, a const char*.
  instance_constructor<const char *> first_constructor = first_reflection.get_constructor<const char *>();
  ...
  boost::reflections::function<const char *> first_function = first_reflection.get_function<const char *>("get_type");

  cout << "First reflection: " << first_function(first_instance) << endl;
 
  // Repeat the steps for the second reflection.
  instance_constructor<const char *> second_constructor = second_reflection.get_constructor<const char *>();
  instance second_instance =   second_constructor("Second Instance");
  boost::reflections::function<const char *> second_function =  second_reflection.get_function<const char *>("get_type");
  cout << "Second reflection: " << second_function(second_instance) << endl;
