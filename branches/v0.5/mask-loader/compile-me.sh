...
CLOUD_COMPONENT_NAME=mask-viewer-standalone
CLOUD_ROOT_DIR=mask_viewer
...
if [ "$KEEP_OLD" = "yes" ]; then
	if [ -d $CLOUD_COMPONENT_NAME ]; then
		...
		echo_run ${SVN_CMD} https://$CLOUD_DISTRO_SITE/svn/branches/v0.5/$CLOUD_COMPONENT_NAME/ $CLOUD_COMPONENT_NAME
	fi
else
