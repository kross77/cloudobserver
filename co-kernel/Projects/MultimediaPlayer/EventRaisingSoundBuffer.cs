...
namespace Mp3Sharp
{
	...
	public class EventRaisingSoundBuffer
	{
		...
		protected void InitSecondaryBuffer()	
		{
			...
			bpn[0].EventNotifyHandle = NotificationEvent.SafeWaitHandle.DangerousGetHandle();

			bpn[1] = new BufferPositionNotify();
			bpn[1].Offset = length-1;
            bpn[1].EventNotifyHandle = NotificationEvent.SafeWaitHandle.DangerousGetHandle();

			bpn[2] = new BufferPositionNotify();
			bpn[2].Offset = (int)PositionNotifyFlag.OffsetStop;
            bpn[2].EventNotifyHandle = NotificationEvent.SafeWaitHandle.DangerousGetHandle();
