...
ifeq ($(config),debug)
  OBJDIR     = bin/obj/Debug/lib-multiple-inheritance-Computer
  TARGETDIR  = bin/debug
  TARGET     = $(TARGETDIR)/liblib-multiple-inheritance-Computer.so
  DEFINES   += -DLIN -DDEBUG
...
ifeq ($(config),release)
  OBJDIR     = bin/obj/Release/lib-multiple-inheritance-Computer
...
ifeq ($(config),debug32)
  OBJDIR     = bin/obj/x32/Debug/lib-multiple-inheritance-Computer
  TARGETDIR  = bin/debug
  TARGET     = $(TARGETDIR)/liblib-multiple-inheritance-Computer.so
  DEFINES   += -DLIN -DDEBUG
  INCLUDES  += -I/opt/local/include -I/usr/include -I/usr/local/include -I../../libs
...
ifeq ($(config),release32)
  OBJDIR     = bin/obj/x32/Release/lib-multiple-inheritance-Computer
  TARGETDIR  = bin/release
  TARGET     = $(TARGETDIR)/liblib-multiple-inheritance-Computer.so
  DEFINES   += -DLIN -DNDEBUG
  INCLUDES  += -I/opt/local/include -I/usr/include -I/usr/local/include -I../../libs
...
$(TARGET): $(GCH) $(OBJECTS) $(LDDEPS) $(RESOURCES)
	@echo Linking lib-multiple-inheritance-Computer
	$(SILENT) $(LINKCMD)
	$(POSTBUILDCMDS)
...
clean:
	@echo Cleaning lib-multiple-inheritance-Computer
ifeq (posix,$(SHELLTYPE))
	$(SILENT) rm -f  $(TARGET)
	$(SILENT) rm -rf $(OBJDIR)
else
...
$(OBJDIR)/computer.o: ../../src/lib-multiple-inheritance/computer.cpp
	@echo $(notdir $<)
	$(SILENT) $(CXX) $(CXXFLAGS) -o "$@" -c "$<"
